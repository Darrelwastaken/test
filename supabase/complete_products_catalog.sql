-- Complete AmBank Products Catalog
-- Run this script in your Supabase SQL Editor to populate all products
...
CREATE INDEX IF NOT EXISTS idx_products_catalog_subcategory ON products_catalog(subcategory);
CREATE INDEX IF NOT EXISTS idx_products_catalog_type ON products_catalog(type);
CREATE INDEX IF NOT EXISTS idx_products_catalog_suitability ON products_catalog(suitability);
CREATE INDEX IF NOT EXISTS idx_products_catalog_islamic ON products_catalog(is_islamic);
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'check_suitability') THEN
...
DROP TRIGGER IF EXISTS trigger_update_products_catalog_updated_at ON products_catalog;
...
SELECT 
    ...
    COUNT(CASE WHEN is_islamic = true THEN 1 END) as islamic_products,
...
SELECT 
    ...
    COUNT(CASE WHEN is_islamic = true THEN 1 END) as total_islamic_products,
